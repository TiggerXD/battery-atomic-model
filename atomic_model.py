...
element_data = {
    "Lithium (Li)": {
        ...
        "protons": 3,
        "neutrons": 4,
        "electrons": [2,1],  # electrons per shell
        "boiling": "1342°C",
        "melting": "180.5°C",
...
if page == "⚛️ Atomic Model":
    ...
    radii = np.linspace(15, 40, len(shells))  # bigger radii for bigger orbits
    n_frames = 60
    frames = []
    ...
    for t in angles:
        frame_data = []
        # Nucleus - bigger balls
        frame_data.append(go.Scatter3d(
            x=[0]*data['protons'], y=[0]*data['protons'], z=[0]*data['protons'],
            mode='markers', marker=dict(size=12,color='red',opacity=0.9), name='Protons'
        ))
        frame_data.append(go.Scatter3d(
            x=[0]*data['neutrons'], y=[0]*data['neutrons'], z=[0]*data['neutrons'],
            mode='markers', marker=dict(size=12,color='blue',opacity=0.9), name='Neutrons'
        ))
        # Electrons
